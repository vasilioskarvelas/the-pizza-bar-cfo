...
|---|---|---|---|---|---|
| Financial Calculations | `base44/workflows/Financial Calculations.jsonc` | `0 6 * * *` (daily 06:00) | Australia/Melbourne | active | UNVERIFIED — run history not observed in the builder; verify externally via get_workflow_run |
| Connector Auto Sync | `base44/workflows/Connector Auto Sync.jsonc` | `0 * * * *` (hourly) | Australia/Melbourne | active | UNVERIFIED — run history not observed in the builder; verify externally via get_workflow_run |
